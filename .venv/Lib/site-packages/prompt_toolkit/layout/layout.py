...
class Layout:
    ...
    def get_focusable_windows(self) -> Iterable[Window]:
        ...
        'model' area).
        """
        for w in self.walk_through_model_area():
    ...
    def get_visible_focusable_windows(self) -> list[Window]:
        ...
        # model container. Make sure to keep the ordering.
    ...
    def walk_through_model_area(self) -> Iterable[Container]:
        """
        Walk through all the containers which are in the current 'model' part
        of the layout.
        """
        # Go up in the tree, and find the root. (it will be a part of the
        # layout, if the focus is in a model part.)
        root: Container = self.current_window
        while not root.is_model() and root in self._child_to_parent:
